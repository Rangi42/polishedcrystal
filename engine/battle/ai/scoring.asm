AICommand_checkturn:
AICommand_checkobedience:
AICommand_usedmovetext:
AICommand_doturn:
AICommand_critical:
AICommand_damagestats:
AICommand_stab:
AICommand_damagevariation:
AICommand_checkhit:
AICommand_lowersub:
AICommand_hittargetnosub:
AICommand_raisesub:
AICommand_failuretext:
AICommand_checkfaint:
AICommand_criticaltext:
AICommand_supereffectivetext:
AICommand_postfainteffects:
AICommand_posthiteffects:
AICommand_poisontarget:
AICommand_sleeptarget:
AICommand_draintarget:
AICommand_eatdream:
AICommand_burntarget:
AICommand_freezetarget:
AICommand_paralyzetarget:
AICommand_selfdestruct:
AICommand_payday:
AICommand_conversion:
AICommand_resetstats:
AICommand_forceswitch:
AICommand_endloop:
AICommand_flinchtarget:
AICommand_recoil:
AICommand_focusenergy:
AICommand_confuse:
AICommand_confusetarget:
AICommand_heal:
AICommand_transform:
AICommand_screen:
AICommand_poison:
AICommand_paralyze:
AICommand_substitute:
AICommand_rechargenextturn:
AICommand_metronome:
AICommand_leechseed:
AICommand_splash:
AICommand_disable:
AICommand_cleartext:
AICommand_charge:
AICommand_checkcharge:
AICommand_traptarget:
AICommand_growth:
AICommand_rampage:
AICommand_checkrampage:
AICommand_constantdamage:
AICommand_counter:
AICommand_encore:
AICommand_painsplit:
AICommand_sketch:
AICommand_sleeptalk:
AICommand_destinybond:
AICommand_falseswipe:
AICommand_healbell:
AICommand_hastarget:
AICommand_triplekick:
AICommand_kickcounter:
AICommand_thief:
AICommand_arenatrap:
AICommand_defrost:
AICommand_curse:
...
AICommand_spikes:
AICommand_foresight:
AICommand_perishsong:
AICommand_startsandstorm:
AICommand_starthail:
AICommand_endure:
AICommand_checkcurl:
AICommand_rolloutpower:
AICommand_conditionalboost:
AICommand_attract:
AICommand_happinesspower:
AICommand_damagecalc:
AICommand_safeguard:
AICommand_checksafeguard:
AICommand_getmagnitude:
AICommand_batonpass:
AICommand_pursuit:
AICommand_clearhazards:
AICommand_healweather:
AICommand_hiddenpower:
AICommand_startrain:
AICommand_startsun:
AICommand_effectchance:
AICommand_switchturn:
AICommand_bellydrum:
AICommand_rage:
AICommand_doubleflyingdamage:
AICommand_doubleundergrounddamage:
AICommand_checkfuturesight:
AICommand_futuresight:
AICommand_doubleminimizedamage:
AICommand_skipsuncharge:
AICommand_thunderaccuracy:
AICommand_teleport:
AICommand_switchout:
AICommand_ragedamage:
AICommand_resettypematchup:
AICommand_allstatsup:
AICommand_raisesubnoanim:
AICommand_lowersubnoanim:
AICommand_clearmissdamage:
AICommand_movedelay:
AICommand_hittarget:
AICommand_tristatuschance:
AICommand_supereffectivelooptext:
AICommand_startloop:
AICommand_curl:
AICommand_burn:
AICommand_bounceback:
AICommand_pickpocket:
AICommand_suckerpunch:
AICommand_toxicspikes:
AICommand_roost:
AICommand_skillswap:
AICommand_trick:
AICommand_knockoff:
...
AICommand_toxic:
AICommand_gyroball:
AICommand_checkpowder:
AICommand_lowkick:
AICommand_brickbreak:
AICommand_trickroom:
AICommand_raisestat:
AICommand_lowerstat:
AICommand_forceraisestat:
AICommand_forcelowerstat:
AICommand_raisestathit:
AICommand_lowerstathit:
AICommand_raiseoppstat:
AICommand_loweroppstat:
AICommand_forceraiseoppstat:
AICommand_forceloweroppstat:
AICommand_raiseoppstathit:
AICommand_loweroppstathit:
; TODO: AI simulation to more easily add AI for new moves.
; A lot of these will reuse BattleCommand scripts, some
; will do nothing (UI-only commands or RNG checks), some
; use custom code for the AI's benefit.
; This will, for example, allow us to more easily figure
; out what to do with new moves, since we can just piggyback
; on the battle engine itself for move simulation.
	ret
