DEF __EVOS_ATTACKS_STATE EQU -1
DEF __EVOS_ATTACKS_LAST_LEVEL EQU -1
DEF __EVOS_ATTACKS_CURRENT_MON EQUS ""

MACRO evo_data
	if __EVOS_ATTACKS_STATE == 0
		db -1 ; end of previous mon's moves
		{__EVOS_ATTACKS_CURRENT_MON}EvosAttacks:
	endc
	assert __EVOS_ATTACKS_STATE != 2, "{__EVOS_ATTACKS_CURRENT_MON} has evo_data after its learnset!"
	REDEF __EVOS_ATTACKS_STATE EQU 1
	db \1 ; evolution type
	if \1 == EVOLVE_PARTY
...
MACRO evos_attacks
	REDEF __EVOS_ATTACKS_CURRENT_MON EQUS "\1"
	assert __EVOS_ATTACKS_STATE != 0, "Empty learnset preceding {__EVOS_ATTACKS_CURRENT_MON}!"
	REDEF __EVOS_ATTACKS_STATE EQU 0
	REDEF __EVOS_ATTACKS_LAST_LEVEL EQU -1
ENDM

MACRO learnset
	if \1 < __EVOS_ATTACKS_LAST_LEVEL
		warn "{__EVOS_ATTACKS_CURRENT_MON} learns \2 at a lower level than previous move!"
	endc
	if __EVOS_ATTACKS_LAST_LEVEL == -1 && \1 != 1
		warn "{__EVOS_ATTACKS_CURRENT_MON} learns its first move at level \1 instead of level 1!"
	endc
	if \1 < 1 || \1 > 100
		warn "{__EVOS_ATTACKS_CURRENT_MON} learns a move at level \1, which should be impossible!"
	endc
	REDEF __EVOS_ATTACKS_LAST_LEVEL EQU \1
	if __EVOS_ATTACKS_STATE != 2
		if __EVOS_ATTACKS_STATE == 0
			{__EVOS_ATTACKS_CURRENT_MON}EvosAttacks:
		endc
		db -1 ; end of evolutions and, if there were no evos, previous mon's moves
	endc
	REDEF __EVOS_ATTACKS_STATE EQU 2
	db \1 ; level
	db \2 ; move
ENDM
