...
; Game Time:
; ----------
; the number of hours the game has been played. This is capped by logic to 999
; hours for purposes of decimal representation. whilst our NoRTC code increases
; the wall clock 6 times faster than normal, the Game Time remains real time

GameSaveTime::      ds 8    ; the in-game wall-clock time at save
    ; = curDay      ; (this appears to be used to see if a day
    ; = hHours      ;  boundary has been passed since last save)
    ; = hMinutes
    ; = hSeconds
    ; ... unused?
wDST::          ds 1
...
; Fake RTC for NoRTC patch:
; -------------------------
; the NoRTC patch is designed for hardware (flash-carts) that do not have
; a Real Time Clock. Because there is no hardware the RTC cannot continue
; ticking when the gameboy is switched off, therefore the NoRTC patch
; compensates by running the in-game clock at 6x speed -- 24 hours in-game
; will occur in 4 hours real-time. this ensures that a player can play through
; day/night cycles without actually having to play 24 continguous hours.
; every second the values from this fake RTC in WRAM are incremented and
; copied to the RTC cache in HRAM used throughout by the game
if DEF(NO_RTC)
wNoRTC::
...
wNoRTCSeconds::     ds 1    ; copied to hRTCSeconds
    ...
    ; reserve the same amount of space in other versions
    ; so that save games remain compatible
    ds 5
endc
