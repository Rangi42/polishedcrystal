...
#define USAGE_OPTS "[-h|--help] [-c|--colors map=012] [-n|--indent px=0] [-s|--spacing px=1] text out.2bpp"
...
void parse_args(int argc, char *argv[], struct Options *options) {
	...
	for (int opt; (opt = getopt_long(argc, argv, "c:n:s:h", long_options)) != -1;) {
		switch (opt) {
		case 'c':
			if (strlen(optarg) != 3) {
...
int main(int argc, char *argv[]) {
	...
	output_2bpp(&output, argv[1], options.colors);
