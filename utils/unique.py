...
"""
Usage: python unique.py [-b|--blanks] [-f|--flip] [-x|--cross] image.png

Erase duplicate tiles from an input image.
-b or --blanks does not erase blank tiles.
...
def unique_tiles(tiles, blanks, flip, cross):
...
def erase_duplicates(filename, blanks, flip, cross):
	with open(filename, 'rb') as file:
		width, height, rows = png.Reader(file).asRGBA8()[:3]
		rows = [list(rgb5_pixels(row)) for row in rows]
	if width % 8 or height % 8:
		return False
	tiles = unique_tiles(rows_to_tiles(rows, width, height), blanks, flip, cross)
...
def main():
	blanks = flip = cross = False
	while True:
		if len(sys.argv) < 2:
			print(f'Usage: {sys.argv[0]} [-b|--blanks] [-f|--flip] [-x|--cross] tileset.png', file=sys.stderr)
			sys.exit(1)
		elif sys.argv[1] in {'-b', '--blanks'}:
			flip = True
		elif sys.argv[1] in {'-f', '--flip'}:
			flip = True
		elif sys.argv[1] in {'-x', '--cross'}:
			cross = True
		elif sys.argv[1] in {'-bf', '-fb'}:
			blanks = flip = True
		elif sys.argv[1] in {'-fx', '-xf'}:
			flip = cross = True
		elif sys.argv[1] in {'-bx', '-xb'}:
			blanks = cross = True
		elif sys.argv[1] in {'-bfx', '-bxf', '-fbx', '-fxb', '-xbf', '-xfb'}:
			blanks = flip = cross = True
		else:
			break
		sys.argv.pop(1)
	for filename in sys.argv[1:]:
		...
		elif not erase_duplicates(filename, blanks, flip, cross):
