...
"""
Usage: python sym_comments.py file.asm [polishedcrystal.sym] > file_commented.asm
...
def main():
	if len(sys.argv) not in {2, 3}:
		print(f'Usage: {sys.argv[0]} file.asm [polishedcrystal.sym] > file_commented.asm', file=sys.stderr)
		sys.exit(1)

	wram_name = sys.argv[1]
	sym_name = sys.argv[2] if len(sys.argv) == 3 else 'polishedcrystal.sym'
