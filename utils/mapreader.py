...
# A library for parsing the polishedcrystal.map file output by rgbds.
...
class MapReader:
	...
	def read_map_data(self, map):
		...
		for line in map:
			line = line.rstrip()
			if self._state == MapReader.INIT:
				if self._parse_init(line):
					self._state = MapReader.BANK

			elif self._state == MapReader.BANK or self._state == MapReader.SECTION:
				if not line:
					self._state = MapReader.INIT
				else:
					line = line.lstrip()
					...
					if parts[0] == 'SECTION':
						...
						self._state = MapReader.INIT
					elif parts[0] == 'EMPTY' and not self._cur_data:
						self._cur_data = {
							'sections': [],
							'used': 0,
							'slack': self._cur_bank_type['size']
						}
						self._state = MapReader.INIT
